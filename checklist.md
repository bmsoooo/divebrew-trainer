...
- [~] 음성 가이드 추상화 계층 + 웹 구현(Web Speech API, 한/영) → verify: iOS Safari·Android Chrome 재생 확인 — 구현·단위 테스트 완료, **실기기 검증 대기** (2026-07-05)
- [~] Wake Lock 세션 중 화면 유지 → verify: iOS Safari에서 10분 세션 동안 화면 꺼지지 않음 — 구현·단위 테스트 완료, **실기기 검증 대기** (2026-07-05)
- [x] 컨트랙션 탭 기록 → verify: 세션 결과에 타임스탬프 저장 확인 — 위젯 테스트 통과 (2026-07-05)
- [x] 세션 완료·기록 저장 → verify: 브라우저 새로고침·재방문 후 히스토리 유지 — DB 저장·재조회 테스트 통과, 실브라우저 새로고침 확인은 M2 배포 시 재검증 (2026-07-05)

## M2. 웹 MVP 마감·배포
- [x] 온보딩 + 안전 동의 게이트 → verify: 미동의 시 진행 불가 — 리다이렉트·부분체크 비활성·영구 저장 테스트 4건 통과 (2026-07-05)
- [x] 세션 시작 안전 리마인더 로테이션 — 세션 수 기반 4문구 로테이션, 첫 라운드 준비 중 표시 (2026-07-05)
- [x] 히스토리 목록 + PB 추이 그래프 (fl_chart) — 세션별 최고 홀드 라인차트 + PB 자동 갱신, 테스트 5건 통과 (2026-07-05)
- [x] 기록 JSON 내보내기/가져오기 (iOS 전환 시 이관 경로) → verify: 내보낸 파일 재가져오기 왕복 테스트 — 왕복·대체·오류 보존 테스트 3건 통과 (2026-07-05)
- [~] PWA 매니페스트 + 홈 화면 추가 동작 확인 — 매니페스트·아이콘·테마 완료, **홈 화면 추가는 실기기 확인 대기** (2026-07-05)
